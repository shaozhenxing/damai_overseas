...
module.exports = function(options) {
  ...
  var config = {
    ...
    resolve: {
      ...
      alias: {
        ...
        'jquery': path.resolve(__dirname, 'app/_vendor/jquery/jQuery-1.11.3'),
        'avalon2': path.resolve(__dirname, 'app/_vendor/avalon/avalon.modern'),
        'utils': path.resolve(__dirname, 'app/_lib/utils'),
        'global': path.resolve(__dirname, 'app/_lib/global'),
        'store': path.resolve(__dirname, 'app/_store/index'),
    ...
    output: {
      ...
      // 目录绝对路径
      // publicPath: '/damai/mobile/english/dist/',
      filename: jsBundle
    },
    module: {
      loaders: [
    ...
    plugins: [
      ...
      new webpack.optimize.CommonsChunkPlugin({
        name: 'common',  // 将公共模块提取，生成名为 `common` 的 chunk
    ...
    // 启用热服务有两种 如果是 api 启动方式, 这里只是一个配置目录,不会被 webpack 读取,
    // 只有命令行才会读取这个参数
    devServer: {
  ...
  pages.forEach(function (pathname) {
    var conf = {
      title: 'Damai.cn',
      filename: './_views/' + pathname + '.html',  // 生成的html存放路径，相对于 path
      template: path.resolve(__dirname, './app/_views/' + pathname + '.ejs'),  // html 模板路径
      hash: true

      /**
        * 压缩这块，调用了 html-minify，会导致压缩时候的很多 html 语法检查问题，
        * 如在 html 标签属性上使用 {{...}} 表达式，很多情况下并不需要在此配置压缩项，
        * 另外，UglifyJsPlugin 会在压缩代码的时候连同 html 一起压缩。
        * 为避免压缩 html，需要在 html-loader 上配置 'html?-minimize'，见 loaders 中 html-loader 的配置。
        */
